...
(function () {
  ...
  /* ---------- insights kategori filtresi (v2.2 — kategori numarasıyla) ---------- */
  (function () {
    var grid = doc.getElementById("ins-grid");
    if (!grid) return;
    var chips = [].slice.call(doc.querySelectorAll(".chip[data-cat]"));
    var cards = [].slice.call(grid.querySelectorAll(".acard[data-cat]"));
    var empty = doc.getElementById("ins-empty");
    ...
    function apply() {
      ...
      cards.forEach(function (card) {
        var on = active === null || card.getAttribute("data-cat") === active;
        card.hidden = !on;
        if (on) shown++;
      });
      ...
      track("insights_filter", { category: active === null ? "all" : active });
    }

    chips.forEach(function (chip) {
      chip.addEventListener("click", function () {
        var v = chip.getAttribute("data-cat");
        active = active === v ? null : v;   /* aynı çipe tekrar dokunmak filtreyi temizler */
        apply();
      });
    });
  })();
